...
class FilterDatabase(database.DatabaseHelper):
    ...
    def get_filter(self, user, name=None, id_=None):
        query = "SELECT id, name, description, value FROM Prewikka_Filter WHERE userid = %(user)s"
        if name:
        ...
        id_, name, description, value = rows[0]
...
class FilterView(FilterPlugin, view.View):
    ...
    def edit(self, name=None):
        id_ = int(env.request.parameters["duplicate"]) if "duplicate" in env.request.parameters else None
    ...
    def save(self):
        ...
        filter_ = self._db.get_filter(env.request.user, filter_old_name) if filter_old_name else None
